...
def ShowFreqHistory():
    ...
    for h in Host.hosts:
        for time in range(len(h.freq_history)):
            freq = h.freq_history[time]

            df = df.append( pd.DataFrame([[time, freq, h.id]], columns=['Timestep', 'Frequency', 'Host']) )

    lm = sns.lmplot(x='Timestep', y='Frequency', hue='Host', data=df)
